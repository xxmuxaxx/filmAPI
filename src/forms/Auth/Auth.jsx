...
let AuthForm = ({handleSubmit, error, valid}) => {
    return (
        <form className="auth-form" onSubmit={handleSubmit}>
            ...
            <Input name="password" type="password" label="Ваш пароль" autoComplete="current-password"/>
            {error && <p className={styles.errorMessage}>{error}</p>}
            <Button color="primary" type="submit" variant="contained" disabled={!valid}>Войти</Button>
        </form>
    )
}
