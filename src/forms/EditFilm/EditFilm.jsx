...
const EditFilm = ({
    ...
    async () => {
      ...
      if (data) {
        ...
        fields.forEach((field) =>
          dispatch(blur('editFilm', `${field}`, data[field]))
        );
      } else {
  ...
  const onSubmit = (formData) => {
    ...
    data.genres = data?.genres
      ? data.genres.split(',').map((genre) => ({ name: genre }))
      : [];
    ...
    filmApi
      ...
      .catch(() =>
        dispatch(
          stopSubmit('editFilm', { _error: 'Ошибка при изменении фильма' })
        )
      );
  };

  return (
    <EditFilmForm onSubmit={onSubmit} initialId={initialId} message={message} />
  );
};
